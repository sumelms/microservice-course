...
const (
	returningColumns = `matrices.uuid, matrices.code, matrices.name, matrices.description,
...
func queriesMatrix() map[string]string {
	return map[string]string{
		...
		getCourseMatrix: fmt.Sprintf(`SELECT
			...
			LEFT JOIN courses ON matrices.course_id = courses.id
			WHERE courses.uuid = $1 AND courses.deleted_at IS NULL
				AND matrices.uuid = $2 AND matrices.deleted_at IS NULL`, returningColumns),
		...
		updateMatrix: fmt.Sprintf(`UPDATE matrices
			...
			RETURNING %s`, returningColumns),
		// DELETE.
		deleteMatrix:  fmt.Sprintf(`UPDATE matrices SET deleted_at = NOW() WHERE uuid = $1 RETURNING %s`, returningColumns),

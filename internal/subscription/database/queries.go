...
func queries() map[string]Query {
	return map[string]Query{
		...
		deleteSubscription: Query("UPDATE subscriptions SET deleted_at = NOW() WHERE id = $1"),
		getSubscription:    Query("SELECT * FROM subscriptions WHERE id = $1"),
		listSubscription:   Query("SELECT * FROM subscriptions"),
		updateSubscription: Query("UPDATE subscriptions SET user_id = $1, course_id = $2, matrix_id = $3, valid_until = $4 WHERE id = $5 RETURNING *"),
	}
}
